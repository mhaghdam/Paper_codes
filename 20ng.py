...
def pre_processing(data, categories):
...
def compared_methods(clusters, X_new, target_new):
    ...
    model = NMF(n_components=clusters, init='random', max_iter=200)
    ...
    model = NMF(n_components=clusters, init='random', solver='mu', beta_loss='kullback-leibler', max_iter=200)
    D = model.fit_transform(X_new)
    W = model.components_
    ...
    brc = Birch(n_clusters=clusters, threshold=.4).fit(X_new)
...
def RANMF_convergence(clusters, X_new, S):
    #### The Proposed RANMF
    Lambeda = 0.05
    converge = []
    before = 0
...
X, target = pre_processing(data, categories)
...
RANMF_convergence(clusters, X_new, S)
